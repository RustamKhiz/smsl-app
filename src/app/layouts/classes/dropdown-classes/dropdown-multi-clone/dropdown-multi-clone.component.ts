...
@Component({
  selector: 'app-dropdown-multi-clone',
  templateUrl: './dropdown-multi-clone.component.html',
  styleUrls: ['./dropdown-multi-clone.component.css']
})
export class DropdownMultiCloneComponent implements OnInit {
  ...
  public setInitialValue() {
    this.filteredItemsMulti
      ...
      .subscribe(() => {
        ...
        this.multiSelect.compareWith = (a: Personals, b: Personals) => a && b && a.Fio === b.Fio;
      });
  }
  ...
  @Output() itemSelectClone: EventEmitter<any> = new EventEmitter();
  NumberId: number[] = []
  FullNameNumberItem: string [] = []
  ...
  SelectionOnChange(event){
    ...
    for (let i = 0; i < event.value.length; i++) {
      ...
      this.FullNameNumber.push(event.value[i].Name)
    }

    console.log("FullNameNumber: ", this.FullNameNumber)

    this.itemSelectClone.emit({valueId: this.NumberId, valueName: this.FullNameNumber})
  }
